...
class FallTemplateBot2025(ForecastBot):
    ...
    async def run_research(self, question: MetaculusQuestion) -> str:
        ...
        async with self._concurrency_limiter:
            # --- BUG FIX ---
            # The 'question' object was missing the '.id' attribute at runtime.
            # This fix parses the ID directly from the 'page_url', which is more robust.
            match = re.search(r'\d+', question.page_url)
            question_id_str = match.group(0) if match else "unknown"
            track_event("Research Started", {"question_id": question_id_str})
            ...
            try:
                ...
                track_event("Research Successful", {"question_id": question_id_str, "summary_length": len(research_summary)})
            except Exception as e:
                error_message = f"Research failed for question {question_id_str}: {e}"
                logger.error(error_message)
                track_event("Research Failed", {"question_id": question_id_str, "error": str(e)})
    ...
    try:
        if run_mode == "tournament":
            ...
            forecast_reports = asyncio.run(
                template_bot.forecast_on_tournament(
                    MetaculusApi.CURRENT_METACULUS_CUP_ID, return_exceptions=This is the main entry point for the forecasting bot.
"""
from __future__ import annotations
